...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: "RarePick",
    koreanTitle: "레어픽",
    subtitle: "특수동물 보호자를 위한 AI 진단 및 병원 추천 서비스",
    type: "Team Ideathon Project",
    period: "2026.05",
    members: "팀 프로젝트",
    role: "백엔드 역할 / 기획 참여",
    roleDescription:
      "저는 RarePick 프로젝트에서 백엔드 쪽 역할을 맡아 서비스 기능 흐름에 필요한 데이터 구조와 API 흐름을 고민했으며, 팀원들과 함께 서비스 기획에도 참여했습니다. 특히 사용자가 증상을 입력한 뒤 AI 진단 결과를 받고, 위치 기반 병원 추천으로 이어지는 전체 흐름이 자연스럽게 연결될 수 있도록 기능 구조를 정리하는 데 집중했습니다.",
    roles: [
      "백엔드 역할 담당",
      "서비스 기획 참여",
      "사용자 문제 정의 참여",
      "특수동물 증상 입력 → AI 진단 → 병원 추천으로 이어지는 기능 흐름 정리",
      "서비스에 필요한 데이터 구조 고민",
      "API 요청 및 응답 흐름 설계 방향 정리",
      "AI 진단 결과와 병원 추천 기능이 자연스럽게 연결되도록 구조 설계",
      "팀원들과 함께 발표 자료 및 서비스 흐름 정리",
      "아이디어톤 발표를 위한 핵심 기능과 차별점 정리 참여",
    ],
    award: "솜커톤 우수상",
    ...
    cardTags: ["Gemini API", "Kakao Maps API", "Backend", "Service Planning"],
    summary:
      "특수동물 보호자를 위한 AI 기반 진단 및 위치 기반 병원 추천 서비스 아이디어톤 프로젝트입니다.",
    description:
      "강아지나 고양이에 비해 특수동물은 관련 정보가 부족하고, 진료 가능한 병원을 찾기 어렵다는 문제가 있습니다. RarePick은 사용자가 반려 특수동물의 종류와 증상을 입력하면 AI가 증상 정보를 분석하고, 위치 기반으로 특수동물 진료가 가능한 병원을 추천하는 서비스 흐름을 기획한 프로젝트입니다.",
    features: [
      "특수동물 종류 선택",
      "증상 입력",
      "AI 기반 증상 분석",
      ...
      "위치 기반 특수동물 병원 추천",
      "병원 위치 지도 표시",
      "AI 진단 결과와 병원 추천 흐름 연결",
      "특수동물 보호자 중심 서비스 구조 설계",
    ],
    highlights: [
      "특수동물 보호자라는 명확한 타깃: 기존 반려동물 서비스는 대부분 강아지와 고양이를 중심으로 구성되어 있습니다. RarePick은 정보 접근성이 낮은 특수동물 보호자를 대상으로 문제를 정의했다는 점에서 차별성을 가졌습니다.",
      "AI 진단과 병원 추천의 연결: 사용자가 증상을 입력하면 AI가 기본적인 증상 정보를 분석하고, 그 결과를 바탕으로 병원 추천 흐름까지 이어지도록 기획했습니다.",
      "백엔드 관점의 기능 흐름 정리: 증상 입력 데이터, AI 진단 결과, 병원 추천 정보가 자연스럽게 연결될 수 있도록 요청과 응답 흐름을 고민했습니다. 아이디어톤 프로젝트였기 때문에 완성된 백엔드 구현보다는 서비스가 실제로 동작하기 위한 데이터 흐름과 기능 구조를 정리하는 데 집중했습니다.",
    ],
    showHighlights: true,
    problems: [
      {
        title: "특수동물 정보 접근성 부족",
        problem: "특수동물은 강아지나 고양이에 비해 질병 정보와 보호자용 정보가 부족합니다.",
        solution: "AI를 활용해 사용자가 입력한 증상을 바탕으로 기본적인 상황 안내를 제공하는 흐름을 기획했습니다.",
      },
      {
        title: "진료 가능한 병원 탐색의 어려움",
        problem: "특수동물 진료가 가능한 병원을 찾기 어렵고, 보호자가 직접 여러 정보를 검색해야 합니다.",
        solution: "Kakao Maps API를 활용해 위치 기반으로 특수동물 병원을 추천하는 흐름을 기획했습니다.",
      },
      {
        title: "기능 간 연결 흐름 설계",
        problem: "AI 진단 결과와 병원 추천 기능이 따로 분리되면 사용자 경험이 끊길 수 있습니다.",
        solution: "백엔드 관점에서 증상 입력, AI 분석 결과, 병원 추천 데이터가 하나의 흐름으로 이어지도록 구조를 고민했습니다.",
      },
    ],
    limitations: [
      "아이디어톤 프로젝트로 실제 서비스 개발보다는 기획과 프로토타입 중심으로 진행되었습니다.",
      "백엔드 기능은 완성된 서버 구현보다는 기능 흐름과 API 구조 구상 중심으로 참여했습니다.",
      "AI 진단 결과의 정확성을 높이기 위해서는 신뢰도 높은 데이터와 전문가 검증이 필요합니다.",
      "병원 데이터의 최신성과 진료 가능 여부 확인이 중요합니다.",
      "실제 서비스화 시 의료 정보 제공 범위에 대한 검토가 필요합니다.",
    ],
    learnings: [
      "RarePick 프로젝트를 통해 짧은 시간 안에 사용자 문제를 정의하고, 해결 방법을 서비스 흐름으로 구체화하는 경험을 할 수 있었습니다.",
      "특히 백엔드 역할을 맡으면서 단순히 화면에 보이는 기능뿐만 아니라, 사용자가 입력한 데이터가 어떤 흐름으로 처리되고 어떤 결과로 이어져야 하는지 고민할 수 있었습니다.",
      "또한 AI API와 지도 API가 각각 따로 존재하는 기능이 아니라, 사용자의 문제 해결 과정 안에서 자연스럽게 연결되어야 한다는 점을 배웠습니다.",
      "아이디어톤을 진행하며 기획, 기능 구조 정리, 발표 준비를 함께 경험했고, 팀원들과 제한된 시간 안에 결과물을 만들어가는 협업의 중요성을 느꼈습니다.",
    ],
    media: [
      {
        type: "youtube",
